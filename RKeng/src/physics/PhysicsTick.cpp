...
namespace RKeng::PhysicsTick
{
    ...
    void Run(PhysicsState& ph, float deltaTime)
    {
        ...
        // Лимит шагов за кадр: если накопилось слишком много (застряли между кубами,
        // лаг, дебаггер) — обрезаем accumulator, иначе получаем спираль смерти.
        // 8 шагов = ~133 мс максимального "долга" при fixedTimestep=1/60.
        static constexpr int MAX_PHYS_STEPS = 8;
        {
            float maxAcc = ph.fixedTimestep * static_cast<float>(MAX_PHYS_STEPS);
            if (ph.accumulator > maxAcc)
            {
                Logger::Warn("[PhysicsTick] accumulator clamped: " +
                             std::to_string(ph.accumulator) + " -> " +
                             std::to_string(maxAcc));
                ph.accumulator = maxAcc;
            }
        }

        int stepIdx = 0;
        while (ph.accumulator >= ph.fixedTimestep && stepIdx < MAX_PHYS_STEPS)
        {
            {
                char buf[64];
                snprintf(buf, sizeof(buf),
